...
$$ \underset{\mathbf{w},\, b,\, \boldsymbol{\xi},\, \boldsymbol{\xi}^{*}}{\min}\; \tfrac{1}{2}\|\mathbf{w}\|^{2} + C \sum_{i=1}^{n} (\xi_{i} + \xi_{i}^{*}) $$
...
\text{s.t.}\;& \sum_{i=1}^{n} (\alpha_i - \alpha_i^*) = 0, \quad 0 \le \alpha_i, \alpha_i^* \le C
\end{aligned}
$$
